...
function spin(){
  ...
  reels_load.forEach(( reel, reel_index )=>{
    setTimeout(()=>{
      ...
      setInterval(() => {
        updateReel( reel, velocidad, result_data.stopPoints[ reel_index ])
      }, 7);
...
function updateReel( reel, velocidad, stop_point ){
  reel.forEach(( reel_obj )=>{
    // finalizar intervalo al matchear los stop points
    if ( (reel[ stop_point ].sprite.position.y != GRID_Y) || isSpining){
