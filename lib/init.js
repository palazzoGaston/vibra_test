...
var textoWinings = new PIXI.Text( 'WIN: $0', estilo );
...
function setup(){
  ...
  lines_container.addChild( createSprite( "line_1" ));
  lines_container.addChild( createSprite( "line_4" ));
  lines_container.addChild( createSprite( "line_5" ));

  lines_container.children[0].visible = false;
  lines_container.children[1].visible = false;
  lines_container.children[2].visible = false;

  lines_container.children[0].x = (WIDTH/2) - (lines_container.children[0].width /2);
  lines_container.children[1].x = (WIDTH/2) - (lines_container.children[1].width /2);
  lines_container.children[2].x = (WIDTH/2) - (lines_container.children[2].width /2);
...
function spin(){
  ...
  while ( result_data.prizes.length === 0 ){
    result_data = wrapper.spin();
  }
...
function verSoloEnGrid( reel_obj ) {
  ...
  if (( reel_obj.sprite.position.y >= ( GRID_Y + GRID_PADDING ) )){
...
function dibujarLinea( prizes ){  
  prizes.forEach(( prize )=>{
    switch ( prize.lineId ){
      case 1:
        lines_container.children[0].position.y = GRID_Y + GRID_PADDING + ( reels_load[0][0].sprite.height / 2 );
        lines_container.children[0].visible = true;
        break;
      case 0:
        lines_container.children[0].position.y = GRID_Y + GRID_PADDING + ( reels_load[0][0].sprite.height * 1.5);
        lines_container.children[0].visible = true;
        break;
      case 2:
